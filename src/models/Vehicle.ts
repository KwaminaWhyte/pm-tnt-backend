...
// Indexes
schema.index({ 'availability.location.city': 1, 'availability.location.country': 1 });
...
schema.virtual('maintenanceStatus').get(function() {
  ...
  const daysUntilService = Math.ceil(
    (this.maintenance.nextService.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)
  );
...
// Methods
schema.methods.isAvailableForDates = function(startDate: Date, endDate: Date): boolean {
...
schema.methods.calculateRentalPrice = function(days: number, insuranceOption?: string): {
  basePrice: number;
  ...
  if (insuranceOption) {
    const insurance = this.rentalTerms.insuranceOptions.find(i => i.type === insuranceOption);
    if (insurance) {
