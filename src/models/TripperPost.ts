...
export interface TripperPostInterface extends Document {
  ...
  addComment(userId: mongoose.Types.ObjectId, text: string): Promise<TripperPostCommentInterface>;
  incrementViewCount(): Promise<TripperPostInterface>;
  incrementShareCount(): Promise<TripperPostInterface>;
  save(userId: mongoose.Types.ObjectId): Promise<TripperPostInterface>;
  unsave(userId: mongoose.Types.ObjectId): Promise<TripperPostInterface>;
  report(userId: mongoose.Types.ObjectId, reason: string): Promise<TripperPostInterface>;
}
...
const tripperPostSchema = new Schema<TripperPostInterface>(
  {
    ...
    hashtags: [{
      ...
      validate: {
        ...
        message: 'Hashtags must contain only letters, numbers, and underscores'
      },
      index: true
    }],
    likes: { 
...
tripperPostSchema.index({ hashtags: 1 });
 ...
 */
tripperPostSchema.methods.like = async function(userId: mongoose.Types.ObjectId): Promise<TripperPostInterface> {
  // Check if already liked
  const alreadyLiked = this.likedBy.some((id: mongoose.Types.ObjectId) => id.equals(userId));
  if (alreadyLiked) return this;
  
  // Remove from dislikedBy if present
  const dislikeIndex = this.dislikedBy.findIndex((id: mongoose.Types.ObjectId) => id.equals(userId));
  if (dislikeIndex >= 0) {
 ...
 */
tripperPostSchema.methods.unlike = async function(userId: mongoose.Types.ObjectId): Promise<TripperPostInterface> {
  const likeIndex = this.likedBy.findIndex((id: mongoose.Types.ObjectId) => id.equals(userId));
  if (likeIndex >= 0) {
 ...
 */
tripperPostSchema.methods.dislike = async function(userId: mongoose.Types.ObjectId): Promise<TripperPostInterface> {
  // Check if already disliked
  const alreadyDisliked = this.dislikedBy.some((id: mongoose.Types.ObjectId) => id.equals(userId));
 ...
 */
tripperPostSchema.methods.undislike = async function(userId: mongoose.Types.ObjectId): Promise<TripperPostInterface> {
  const dislikeIndex = this.dislikedBy.findIndex((id: mongoose.Types.ObjectId) => id.equals(userId));
  if (dislikeIndex >= 0) {
    this.dislikedBy.splice(dislikeIndex, 1);
    this.dislikes = Math.max(0, this.dislikes - 1);
 ...
 */
tripperPostSchema.methods.incrementViewCount = async function(): Promise<TripperPostInterface> {
  this.viewCount += 1;
  return this.save();
};
 ...
 */
tripperPostSchema.methods.incrementShareCount = async function(): Promise<TripperPostInterface> {
  this.shareCount += 1;
  return this.save();
};
 ...
 */
tripperPostSchema.methods.save = async function(userId: mongoose.Types.ObjectId): Promise<TripperPostInterface> {
  // Check if already saved
  const alreadySaved = this.savedBy.some((id: mongoose.Types.ObjectId) => id.equals(userId));
 ...
 */
tripperPostSchema.methods.unsave = async function(userId: mongoose.Types.ObjectId): Promise<TripperPostInterface> {
  const saveIndex = this.savedBy.findIndex((id: mongoose.Types.ObjectId) => id.equals(userId));
  if (saveIndex >= 0) {
...
tripperPostSchema.methods.report = async function(
  ...
  const alreadyReported = this.reportedBy.some((id: mongoose.Types.ObjectId) => id.equals(userId));
 ...
 * Find popular posts
 */
tripperPostSchema.statics.findPopular = async function(limit: number = 10): Promise<TripperPostInterface[]> {
 ...
 */
tripperPostSchema.statics.findByLocation = async function(location: string): Promise<TripperPostInterface[]> {
  return this.find({
    isPrivate: false,
    isArchived: false,
    $or: [
...
try {
  TripperPost = mongoose.model<TripperPostInterface, TripperPostModel>("TripperPost");
} catch (error) {
  TripperPost = mongoose.model<TripperPostInterface, TripperPostModel>("TripperPost", tripperPostSchema);
}
